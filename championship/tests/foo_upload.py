...
from championship.views import clean_name, CreateResultsView
from championship.forms import AddTop8ResultsForm, SubmitButtonMixin
...
class EventLinkImportTestCase(TestCase):
    ...
    def test_tournament_validation(self):
        self.login()
        premier_event = EventFactory(
            organizer=self.organizer,
            date=datetime.date.today(),
            category=Event.Category.PREMIER,
        )
        self.data["event"] = premier_event.id
        resp = self.client.post(reverse("results_create_eventlink"), self.data)
        self.assertEqual(400, resp.status_code)
        self.assertIn(
            "SUL Premier events require at least 17 players.", resp.content.decode()
        )
        results = EventPlayerResult.objects.filter(event=premier_event)
...
class ManualImportTestCase(TestCase):
    ...
    def test_tournament_validation(self):
        ...
        self.assertIn("Player Antoine Albertelli has too many match points.", content)
        self.assertIn(
